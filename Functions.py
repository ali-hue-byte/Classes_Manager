# Helper functions for encryption, data storage, and UI animations
...
USER = "users.pkl"         # user information

# Connect to the SQLite database and create a cursor
conn = sqlite3.connect("data.db")
c = conn.cursor()

# Helper functions for managing students and classes in the SQLite database
...
# Animate a Qt widget (title) moving from start to end position.
def animate_title(title, start, end):
    if hasattr(title, "anim"):    # Checks if title has attribute anim
        title.anim.stop()         
...
# Load user data from the pickle file, returns a list of users
def load_data():
...
# Saves user data to the pickle file
def save_data(data):
...
#Helper functions for encryption, decryption and password hashing
def KDF2(password, salt):
    e_password = password.encode() if isinstance(password, str) else password
    kdf = PBKDF2HMAC(algorithm=hashes.SHA256(), length=32, salt=salt, iterations=100000)  # SHA-256 Algorithm for hashing
...
def decrypt_data(data, fernet):
    if isinstance(data, str): #Checks if data is a string 
        data = data.encode()
